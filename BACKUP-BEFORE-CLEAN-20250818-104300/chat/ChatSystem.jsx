...
const ChatSystem = () => {
  ...
  const initializeChatSystem = async () => {
    try {
      const promises = [loadRooms(), setupRealtimeSubscriptions()];
  ...
  const loadRooms = async () => {
    try {
      // 🚨 EMERGENCY: Use only mock data to bypass database issues
      console.log(
        "🚀 EMERGENCY: Using mock rooms data to bypass database schema issues"
      );
      ...
      const mockRooms = [
        {
          id: "mock-room-1",
          name: "General Chat",
          description: "Welcome to SnakkaZ! Start chatting here.",
          is_active: true,
        ...
        },
        {
          ...
          description: "Discuss the latest in Norwegian tech",
          is_active: true,
          created_at: new Date().toISOString(),
          created_by: "demo-user",
          member_count: 27,
        ...
        },
        {
          ...
          description: "Model Context Protocol development chat",
          is_active: true,
          created_at: new Date().toISOString(),
          created_by: "demo-user",
          member_count: 15,
      ...
      setRooms(mockRooms);
      console.log("✅ Mock rooms loaded:", mockRooms.length, "rooms");
      return; // Skip database queries entirely

      // OLD DATABASE CODE (commented out to avoid schema issues)
      /*
      ...
      */
      ...
      console.error("Error in loadRooms (mock mode):", error);
      // Even in error, provide mock data
      setRooms([
        {
          id: "emergency-room",
          name: "Emergency Chat",
          description: "Emergency fallback room",
          is_active: true,
          created_at: new Date().toISOString(),
          created_by: "system",
          member_count: 1,
  ...
  const initializeMcpConnection = async () => {
    ...
    try {
      ...
      if (queryError) {
        console.warn(
          "MCP connections table not available:",
          queryError.message
        );
        return;
      }
      ...
      if (
        existingConnections &&
        existingConnections.length >= MCP_SECURITY_CONFIG.maxConnectionsPerUser
      ) {
  ...
  const loadMessages = async () => {
    ...
    // EMERGENCY MOCK MESSAGES - BYPASS DATABASE
    console.log("🔥 FORCING MOCK MESSAGES FOR UI TESTING");
    const mockMessages = [
      {
        id: "msg_1",
        ...
        profiles: {
          username: "admin",
          display_name: "SnakkaZ Admin",
          avatar_url: null,
        },
      },
      {
        id: "msg_2",
        content: "Dette er et test av den nye glass/liquid design systemet ✨",
        author_id: "user_test",
        room_id: currentRoom.id,
      ...
      {
        id: "msg_3",
        content: "MCP integrasjon fungerer perfekt! 💎",
        author_id: "user_mcp",
        room_id: currentRoom.id,
      ...
      {
        id: "msg_4",
        content: "Real-time chat er nå live og production-ready! 🌟",
        author_id: "user_beta",
        room_id: currentRoom.id,
    ...
    // ORIGINAL DATABASE CODE (DISABLED FOR EMERGENCY)
    /*
    try {
      const { data, error } = await supabase
        .from('messages')
  ...
  const loadParticipants = async (roomId) => {
    // EMERGENCY MOCK PARTICIPANTS - BYPASS DATABASE
    console.log("🔥 FORCING MOCK PARTICIPANTS FOR UI TESTING");
    const mockParticipants = [
      {
        id: "part_1",
        room_id: roomId,
        profile_id: "user_admin",
        is_active: true,
        profiles: {
          username: "admin",
          display_name: "SnakkaZ Admin",
    ...
    console.log("✅ Mock participants loaded:", mockParticipants.length);
  ...
  const sendMessage = async (e) => {
    ...
    // EMERGENCY MOCK MESSAGE SENDING - BYPASS DATABASE
    console.log("🔥 FORCING MOCK MESSAGE SEND FOR UI TESTING");
    ...
    setMessages((prevMessages) => [...prevMessages, mockNewMessage]);
    setNewMessage("");
    console.log("✅ Mock message sent:", mockNewMessage.content);
            ...
            <button
              ...
              className={`w-full text-left p-3 hover:bg-slate-700 border-b border-slate-700 transition-colors ${
                currentRoom?.id === room.id
                  ? "bg-slate-700 border-l-2 border-l-blue-500"
                  : ""
              }`}
            >
