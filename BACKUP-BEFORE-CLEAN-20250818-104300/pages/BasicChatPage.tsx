              ...
              <ScrollArea className="flex-1 p-4">
                <div className="space-y-4">
                  {messages.map((msg) => (
                    <div
                      key={msg.id}
                      className={`flex flex-col gap-1 ${msg.type === 'user' && msg.user === (user.email?.split('@')[0] || 'Anonym')
                          ? 'items-end'
                          : 'items-start'
                        }`}
                    >
                      <div className="flex items-center gap-2 text-xs text-cyberdark-400">
                        <span className={`font-medium ${msg.type === 'welcome' ? 'text-cybergold-400' :
                      ...
                      <div
                        className={`max-w-xs px-4 py-2 rounded-lg ${msg.type === 'user' && msg.user === (user.email?.split('@')[0] || 'Anonym')
