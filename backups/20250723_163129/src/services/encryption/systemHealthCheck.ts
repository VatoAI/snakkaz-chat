...
/**
 * Test if Namecheap Analytics is loading correctly
 */
export function testNamecheapAnalytics() {
  console.log('Testing Namecheap Analytics loading...');
  ...
  // Check if the global snakkazAnalytics object exists (indicates Namecheap Analytics loaded)
  ...
  if (analyticsScripts.length > 0) {
    if (analyticsObjectExists) {
      console.log('Namecheap Analytics is loaded and functioning');
      return { 
        loaded: true,
      ...
      console.log('Namecheap Analytics script exists but may not be properly loaded');
      return {
        loaded: false,
        script: analyticsScripts[0].src,
        objectExists: false,
    ...
    console.log('No Namecheap Analytics script found');
    // Try to load it now
 ...
 */
/**
 * Check if Namecheap DNS and services are properly configured for the domain
 * Also verifies SSL status and key Namecheap protections
...
export function checkNamecheapStatus(): Promise<{success: boolean, details?: Record<string, unknown>}> {
  ...
  // Try to check if we're behind Namecheap by looking for CF specific headers
  return fetch('https://www.snakkaz.com/api/dns-check', { 
    method: 'GET',
    cache: 'no-store'
  })
    .then(response => {
      // Check if Namecheap-specific headers are present
      const ncRay = response.headers.get('nc-ray');
      const ncCache = response.headers.get('nc-cache-status');
      const hasNamecheapHeaders = !!ncRay;
      
      return response.text().then(text => {
        return { text, ncRay: ncRay || '', ncCache: ncCache || '', hasNamecheapHeaders };
      });
    })
    .then((result: { text: string, ncRay: string, ncCache: string, hasNamecheapHeaders: boolean }) => {
      const { text, ncRay, ncCache, hasNamecheapHeaders } = result;
      ...
      const isNamecheapDns = hasNamecheapHeaders || text.includes('nc-ray=') || text.includes('region=');
      ...
      if (isNamecheapDns) {
        ...
        console.log('Namecheap datacenter:', traceData.colo || 'Unknown');
        console.log('NC-Ray ID:', ncRay || traceData.ray || 'Unknown');
        console.log('Cache status:', ncCache || 'Unknown');
          ...
          .then(isReachable => {
            return { 
              ...
              details: { 
                ...
                recommendation: 'Verify Namecheap nameservers are properly configured in domain registrar'
              }
            };
          });
      }
    ...
    .catch(error => {
      console.error('Error checking Namecheap status:', error);
...
/**
 * Run additional network diagnostics for Namecheap
 */
async function runNamecheapNetworkDiagnostics(): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    await fetch('https://namecheap.com', { method: 'HEAD' });
...
export function runSystemHealthCheck(): Promise<{
  ...
  details: {
    ...
    metaTags: boolean;
    namecheapAnalytics: boolean;
    namecheapDns: boolean;
  ...
  const results = {
    ...
    details: {
      ...
      dnsHealth: {
        ...
        namecheap: {
          usingNamecheapNameservers: false,
          nameservers: []
        }
      }
    }
  ...
  // Check Namecheap Analytics 
  const snakkazAnalyticsResult = testNamecheapAnalytics();
  results.details.namecheapAnalytics = snakkazAnalyticsResult.loaded;
  
  // Test Namecheap DNS configuration
  return checkNamecheapStatus()
    .then(ncDnsResult => {
      results.details.namecheapDns = ncDnsResult.success;
    ...
    .then(pingBlockingResult => {
      ...
      return dnsManager.performHealthCheck().then(dnsHealth => {
        ...
        // Overall health determination - don't require Namecheap DNS to be active yet
...
export async function checkDnsHealth(
  ...
  try {
    ...
    if (!namecheapApiKey && !namecheapApiToken) {
      return {
        success: false,
        details: {
          status: 'issues',
          ...
          recommendations: [
            'Provide Namecheap API key and/or Namecheap API token to perform a complete DNS check',
    ...
    await dnsManager.initialize(namecheapApiKey || '', namecheapApiToken || '');
    ...
    return {
      ...
      details: {
        ...
        nameserversConfigured: healthResult.namecheap.usingNamecheapNameservers,
        zoneActive: healthResult.namecheap.zoneActive
      }
    };
  } catch (error) {
    ...
    return {
      ...
      details: {
        ...
        recommendations: [
          ...
          'Ensure network connectivity to Namecheap and Namecheap APIs',
...
export async function runComprehensiveHealthCheck(
  ...
  const [cspResult, dnsResult, ncResult, securityResult] = await Promise.all([
    ...
    checkDnsHealth(options?.namecheapApiKey, options?.namecheapApiToken),
  ...
  if (!ncResult.success) {
    issues.push('Namecheap integration has issues');
    if (ncResult.details && typeof ncResult.details === 'object' && 'message' in ncResult.details) {
      issues.push(String(ncResult.details.message));
    }
  }
  ...
  if (dnsResult.details.status === 'critical' || !ncResult.success || !securityResult.success) {
  ...
  setTimeout(() => {
    runSystemHealthCheck()
      .then(results => {
        ...
        if (!results.healthy) {
          // Try to fix issues
          if (!results.details.namecheapAnalytics) {
            import('./analyticsLoader').then(module => {
              module.loadNamecheapAnalytics();
            });
          }
        }
      })
