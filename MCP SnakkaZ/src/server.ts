...
class ConnectionManager {
  ...
  addConnection(userId: string, connectionId: string, socket: any) {
...
class SnakkaZMCPServer {
        ...
        tools: [
          {
            ...
            description:
              "Get the current status of SnakkaZ chat system with detailed metrics",
            inputSchema: {
              type: "object",
              properties: {},
            },
            ...
            inputSchema: {
              ...
              properties: {
                ...
                video_enabled: {
                  type: "boolean",
                  description:
                    "Whether video is enabled (false for audio-only)",
                },
              },
          ...
          },
          {
            ...
            description:
              "Get real-time analytics and server performance metrics",
            inputSchema: {
              type: "object",
              properties: {
                include_details: {
          ...
          },
          {
            ...
            description:
              "Send system-wide broadcast message to all connected users",
            inputSchema: {
              type: "object",
              properties: {
                message: {
        ...
        switch (name) {
          ...
          case "send_message":
            ...
            return await this.sendMessage(
              ...
              String(args.message),
              args.priority ? String(args.priority) : "normal"
            );
          ...
          case "send_notification":
            ...
            return await this.sendNotification(
              ...
              String(args.title),
              String(args.body),
              args.type ? String(args.type) : "message"
            );
          ...
          case "get_realtime_analytics":
            return await this.getRealtimeAnalytics(
          ...
          case "manage_user_connection":
            ...
            return await this.manageUserConnection(
              String(args.action),
              args.userId ? String(args.userId) : undefined
            );

          case "broadcast_system_message":
            if (!args)
              throw new Error("Missing arguments for broadcast_system_message");
            return await this.broadcastSystemMessage(
              String(args.message),
              String(args.type),
              args.persist === true
            );
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching chat status: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
  ...
  private async sendMessage(
    ...
    try {
      ...
      console.log(
        `[MCP] Message sent - ID: ${messageId}, To: ${recipient}, Priority: ${priority}, Time: ${timestamp}`
      );
  ...
  private async fetchChatMetrics() {
  ...
  private async sendNotification(
    ...
    try {
      ...
      console.log(
        `[MCP] Notification sent - ID: ${notificationId}, To: ${recipient}, Type: ${type}`
      );
  ...
  private async startVideoCall(
    ...
    try {
      ...
      console.log(
        `[MCP] Video call initiated - ID: ${callId}, From: ${caller}, To: ${recipient}, Video: ${videoEnabled}`
      );
  ...
  private async getUploadFileStatus(username: string) {
    try {
  ...
  private async getRealtimeAnalytics(includeDetails: boolean = false) {
    try {
      ...
      const basicAnalytics = {
        ...
        connections: {
          ...
          websocket_connections: activeUsers.reduce(
            (total, userId) =>
              total + this.connectionManager.getUserConnectionCount(userId),
            0
          ),
        },
  ...
  private async manageUserConnection(action: string, userId?: string) {
    try {
      switch (action) {
        case "list":
          const activeUsers = this.connectionManager.getActiveUsers();
                ...
                text: JSON.stringify(
                  {
                    ...
                    connection_details: activeUsers.map((id) => ({
                      userId: id,
                      connections:
                        this.connectionManager.getUserConnectionCount(id),
                      status: "connected",
                    })),
        ...
        case "status":
          if (!userId) throw new Error("userId required for status action");
        ...
        case "disconnect":
          if (!userId) throw new Error("userId required for disconnect action");
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error managing user connection: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error broadcasting system message: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
