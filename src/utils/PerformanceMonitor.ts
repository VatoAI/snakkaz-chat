...
export class SnakkazPerformanceMonitor {
  ...
  public getPerformanceGrade(): 'A' | 'B' | 'C' | 'D' | 'F' {
    ...
    // Grading specifically for Norwegian tech community expectations
    if (lcp <= 2000 && fid <= 50 && cls <= 0.05 && loadTime <= 3000) return 'A';
    if (lcp <= 2500 && fid <= 100 && cls <= 0.1 && loadTime <= 4000) return 'B';
    if (lcp <= 3000 && fid <= 200 && cls <= 0.15 && loadTime <= 5000) return 'C';
    if (lcp <= 4000 && fid <= 300 && cls <= 0.25 && loadTime <= 7000) return 'D';
    return 'F';
  }
