/**
 * SNAKKAZ SERVICE WORKER - OPTIMIZED NOTIFICATION & CACHE SYSTEM
 * Enhanced service worker with improved push notifications and caching
 */
...
self.addEventListener("install", (event) => {
  console.log("[SW] Installing SnakkaZ Service Worker v2...");
...
if (workbox) {
  ...
  workbox.routing.registerRoute(
    ({ url }) =>
      url.pathname.includes("/chat") || url.pathname.includes("/messages"),
    new workbox.strategies.NetworkFirst({
  ...
  self.addEventListener("install", (event) => {
    console.log(
      "[SW] Installing SnakkaZ FASE 6 Service Worker (fallback mode)"
    );
    event.waitUntil(
        ...
        fetch(event.request)
          ...
          .catch(() => {
            return caches.match(event.request).then((cachedResponse) => {
              return (
                cachedResponse ||
                new Response('{"error": "Offline", "cached": false}', {
...
async function handleShareTarget(request) {
  ...
  const chatUrl = `/beta-chat?share=true&data=${btoa(
...
function handleDigitalVokterAlert(alertData) {
  ...
  if (alertData.severity === "high" || alertData.severity === "critical") {
    self.registration.showNotification("🛡️ Sikkerhetsmerknad", {
      body:
        alertData.message ||
        "Digital Vokter har oppdaget en sikkerhetshendelse",
...
self.addEventListener("install", (event) => {
  console.log(
    "[SW] SnakkaZ FASE 6 Service Worker installing - PWA Excellence Mode"
  );
...
console.log(
  "[SW] SnakkaZ FASE 6 Service Worker loaded - PWA Excellence & Digital Vokter Ready! 🚀"
);
