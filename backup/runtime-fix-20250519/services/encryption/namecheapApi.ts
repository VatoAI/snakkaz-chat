 ...
 * and ensure proper configuration with Cloudflare DNS.
 */
...
export class NamecheapApi {
  ...
  /**
   * Check if the domain is using Cloudflare nameservers
  ...
  async isUsingCloudflareNameservers(): Promise<boolean> {
    try {
      ...
      // Check if using Cloudflare nameservers (typically *.ns.cloudflare.com)
      return nameservers.some(ns => ns.includes('cloudflare.com'));
    } catch (error) {
      console.error("Error checking Cloudflare nameservers:", error);
      return false;
    }
  }
  
  /**
   * Configure the domain to use Cloudflare nameservers
   * @returns Success status
   */
  async setCloudflareNameservers(): Promise<boolean> {
    ...
    try {
      return await this.setCustomDns(cloudflareNameservers);
    } catch (error) {
